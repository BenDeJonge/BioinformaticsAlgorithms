...
from Chapter02.Ex_02_02_08_MotifEnumeration.motif_enumeration \
    import motif_enumeration as fun
...
if __name__ == '__main__':
    ...
    for file in inputs:
        with open(file, 'r') as f:
            lines = f.readlines()
            length, mismatches = lines[0].split()
            genes = lines[1].split()
        r = fun(int(length), int(mismatches), genes)
        if len(r) > 0:
